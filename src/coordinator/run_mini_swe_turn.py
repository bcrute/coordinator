...
def build_command(
    ...
    # mini replaces its default config list as soon as -c is used.  Always name
    # either the operator's base config or mini.yaml before bounded overrides.
    command.extend(("--config", str(args.config) if args.config else "mini.yaml"))
...
trajectory. Codex must review the actual diff and independently run relevant tests.
"""
...
def run(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        write_status(
            ...
            review_round,
            state,
            "The bounded mini-swe-agent handoff ended; Codex review is required.",
            blocker,
        )
        write_progress(
            repo,
    ...
    if returncode != 0:
        ...
        return returncode
    print("mini-swe-agent handoff ended. Codex review is required before another turn.")
    return 0
