...
| SOL-008 | 6 | executor efficiency | high | yes | A bounded Qwen response ran with `max_tokens=-1` and exceeded 17,000 decoded tokens without producing a tool action or product edit. | Bounded mini-swe commands now cap each response at 4,096 tokens, allowing the existing length-recovery prompt to request a concise tool call. | `tests.test_executor_adapters.MiniTrajectoryTests.test_command_uses_default_config_then_bounded_overrides` | Pending cycle 7 |
| SOL-009 | 6 | process lifecycle | high | yes | App shutdown killed the nested mini runner before its `finally` block could persist blocked state and remove its turn lock. | Process supervision now gives saved descendant groups a bounded TERM cleanup grace before SIGKILL escalation. | `tests.test_process_guard.ProcessGuardTests.test_session_escaping_descendant_gets_term_cleanup_grace` | Pending cycle 7 |
