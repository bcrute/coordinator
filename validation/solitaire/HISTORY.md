...
| SOL-005 | 4 | process lifecycle | high | yes | Gracefully stopping Coordinator terminated its app-owned watcher but left `runtime/watcher-executor.lock` with the dead watcher PID. | WatcherManager now reclaims only a confirmed-stale lock after stop and shutdown, preserving any live or replaced owner. | `tests.test_workflow.WatcherControlTests.test_stop_reclaims_the_exact_dead_watcher_lock` | Pending cycle 5 |
