...
| Subprocess ownership | Parent-death supervision, child/grandchild termination, exit-status propagation, and PTY signal forwarding | `tests.test_process_guard`, `tests.test_codex_session` |
...
| Executor adapters | Default Claude compatibility, trusted model/effort selection, independent reviewer/supervisor/subagent effort propagation, bounded command construction, and diagnostics | `tests.test_executor_adapters`, `tests.test_executor_settings`, `tests.test_workflow_runners` |
...
| MCP local delegation | Explicit routing threshold, safe path/test inputs, structured tool schema, worktree isolation, patch-only return, independent validation, violations, cleanup, and dashboard state | `tests.test_delegation`, `tests.test_executor_settings`, `tests.test_web_workflow_state` |
| Provider allowance display | Provider parsing/cache failures, reset-scoped velocity persistence, rolling/fallback projections, remaining-window rendering, and manual refresh | `tests.test_provider_usage`, Chromium journey |
